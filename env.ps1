# env.ps1 - terraform-provider-pyvider Development Environment Setup
#
...
if (-not (Test-Path "pyproject.toml")) {
    ...
    Write-Host "Please run this script from the terraform-provider-pyvider root directory"
    exit 1
}
...
$Profile = if ($env:TERRAFORM_PROVIDER_PYVIDER_PROFILE) { $env:TERRAFORM_PROVIDER_PYVIDER_PROFILE } else { "default" }
if ($Profile -eq "default") {
    $VenvDir = "workenv/terraform-provider-pyvider_${TFOS}_${TFARCH}"
} else {
...
$LogDir = Join-Path $env:TEMP "terraform-provider-pyvider_setup"
...
Write-Host "Installing terraform-provider-pyvider in editable mode..." -NoNewline
try {
    & uv pip install --no-deps -e . 2>&1 | Out-File -FilePath (Join-Path $LogDir "install.log")
    Write-Success " terraform-provider-pyvider installed"
}
catch {
...
# Sibling with configuration
# Explicit sibling
$supsrcDir = Join-Path $ParentDir "supsrc"
if (Test-Path $supsrcDir) {
    $WithDeps = $true    $DepsText = if ($WithDeps) { " with dependencies" } else { " without dependencies" }
    Write-Host "Installing supsrc$DepsText..." -NoNewline
    try {
        if ($WithDeps) {
            & uv pip install -e $supsrcDir
        } else {
            & uv pip install --no-deps -e $supsrcDir
        }
        Write-Success " supsrc installed"
        $SiblingCount++
    }
    catch {
        Write-Warning " Failed to install supsrc package from '$supsrcDir'"
        Write-Host "Attempting to continue..."
    }
}
...
Write-Host "`n$("terraform-provider-pyvider development environment activated" | Write-Host -ForegroundColor Green)"
Write-Host "Virtual environment: $VenvDir"
Write-Host "Profile: $Profile"
Write-Host "`nUseful commands:"
Write-Host "  terraform-provider-pyvider --help  # terraform-provider-pyvider CLI"

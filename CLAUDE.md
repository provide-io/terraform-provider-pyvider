...
│   ├── guides/           # Copied from guides/ during build
...
│   └── index.md          # Generated by build script
```

**Documentation Generation Workflow**:

1. **Source Guides**: Manually-written guides live in `guides/` directory at repository root
   - These files are version controlled and should be edited directly
   - Examples: getting started tutorials, common patterns, troubleshooting guides

2. **Build Process** (`make docs` or `scripts/build-docs.sh`):
   - Step 1: Copy `guides/*.md` → `docs/guides/`
   - Step 2: Run plating to generate component documentation → `docs/resources/`, `docs/data-sources/`, `docs/functions/`
   - Step 3: Plating automatically discovers `docs/guides/` and includes in navigation

3. **Generated Output**: The `docs/` directory contains all generated documentation
   - Currently checked into git for simplicity
   - Plating's `MkdocsNavGenerator` automatically creates navigation including guides
   - Guides support frontmatter: `guide_order` (for ordering), `page_title` (for display name)

**Why This Architecture?**:
- ✅ **Clear separation**: Source (`guides/`) vs. generated (`docs/`) content in different directories
- ✅ **Prevents accidents**: No risk of plating overwriting manually-created guides
- ✅ **Better DX**: Obvious where to edit guide content
- ✅ **Git-friendly**: Edit `guides/`, commit changes, `docs/guides/` regenerates automatically
- ✅ **Terraform Registry compatible**: Final structure in `docs/` matches registry expectations

**Adding New Guides**:
1. Create markdown file in `guides/` directory (e.g., `guides/my-guide.md`)
2. Optionally add frontmatter for ordering and title:
   ```markdown
   ---
   guide_order: 10
   ...
   ---
   ```
3. Run `make docs` to copy guide to `docs/guides/` and regenerate navigation
4. Plating automatically includes the guide in mkdocs.yml navigation

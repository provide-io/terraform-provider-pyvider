...
# This script:
# - Copies guides from guides/ to docs/guides/ (source guides preserved)
# - Uses Plating to generate docs from pyvider-components code
...
- **Plating**: New documentation generation system (use `from plating import Plating`)
- All scripts, Makefile targets, and CI/CD workflows now use plating
- Plating provides programmatic access to documentation generation
...
# - Generates documentation using Plating
...
**IMPORTANT**: Documentation follows a clean separation between source content and generated output.

**Directory Structure**:
```
terraform-provider-pyvider/
├── plating/               # Plating templates and source content
│   ├── guides/           # Source: Manually-written guides (EDIT HERE)
│   ├── partials/         # Reusable documentation snippets
│   └── templates/        # Provider-level documentation templates
├── docs/                  # Generated: Auto-generated documentation (DO NOT EDIT)
│   ├── guides/           # Copied from plating/guides/ during build
│   ├── resources/        # Generated by plating from pyvider-components
│   ├── data-sources/     # Generated by plating from pyvider-components
│   ├── functions/        # Generated by plating from pyvider-components
│   └── index.md          # Generated from plating templates
```

**Documentation Generation Workflow**:

1. **Source Guides**: Manually-written guides live in `plating/guides/` directory
   - These files are version controlled and should be edited directly
   - Examples: getting started tutorials, provider tour, troubleshooting guides

2. **Build Process** (`make docs` or `plating plate`):
   - Step 1: Plating reads templates from `plating/` directory
   - Step 2: Plating copies `plating/guides/*.md` → `docs/guides/`
   - Step 3: Plating generates component docs from pyvider-components → `docs/resources/`, `docs/data-sources/`, `docs/functions/`
   - Step 4: Plating processes templates and creates examples

3. **Generated Output**: The `docs/` directory contains all generated documentation
   - Currently checked into git for visibility
   - Plating automatically includes guides in navigation
   - Guides support frontmatter: `guide_order` (for ordering), `page_title` (for display name)

**Why This Architecture?**:
- ✅ **Clear separation**: Source (`plating/`) vs. generated (`docs/`) content
- ✅ **Plating integration**: Guides and templates live alongside plating configuration
- ✅ **Better DX**: All documentation source in one place (`plating/`)
- ✅ **Git-friendly**: Edit `plating/guides/`, run `plating plate`, documentation regenerates
- ✅ **Terraform Registry compatible**: Final structure in `docs/` matches registry expectations

**Adding New Guides**:
1. Create markdown file in `plating/guides/` directory (e.g., `plating/guides/05-my-guide.md`)
2. Add frontmatter for ordering and title:
   ```markdown
   ---
   guide_order: 5
   page_title: "My Custom Guide Title"
   ---
   ```
3. Run `plating plate` to copy guide to `docs/guides/` and regenerate documentation
4. Plating automatically includes the guide in navigation
...
2. Use `Plating` class (from `plating import Plating`) instead of garnish CLI commands

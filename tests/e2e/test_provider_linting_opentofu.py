...
def tofu_binary(tmp_path_factory: pytest.TempPathFactory) -> Path:
    cache = tmp_path_factory.mktemp("opentofu-beta")
    completed = subprocess.run(
        [str(INSTALLER), "--cache-dir", str(cache)],
        check=True,
        capture_output=True,
        text=True,
    )
    tofu = Path(completed.stdout.strip())
...
def tofu_env(cli_config: Path, data_dir: Path, selector: str | object = UNSET) -> dict[str, str]:
    env = os.environ.copy()
    env.pop("PYVIDER_LINT", None)
    env.pop("PYVIDER_CONFIG_FILE", None)
    env.update(
        {
...
def validate_case(
    ...
    def _validate(
        *,
        ...
        workspace.mkdir()
        shutil.copy2(FIXTURE / "main.tf", workspace / "main.tf")
...
def assert_json_location(rule: str, diagnostic: dict[str, Any]) -> None:
    ...
    assert diagnostic["summary"].endswith(f"({rule})")
